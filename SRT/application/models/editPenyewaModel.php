...
class editPenyewaModel extends CI_Model {

	public function edtProfile(){
    $data = [
        "idPenyewa" => $this->input->post('id',true),
        "namaPenyewa" => $this->input->post('nama', true),
        "alamatPenyewa" => $this->input->post('alamat', true),
    ];
    $this->db->where('idPenyewa = ',$data['idPenyewa']);
    $this->db->update('penyewa', $data);
    $this->session->set_userdata('namaPenyewa', $data['namaPenyewa']);
    $this->session->set_userdata('alamatPenyewa', $data['alamatPenyewa']);
    ...
    public function edtPassProfile(){
        ...
        $cek = $this->db->get_where('penyewa',['idPenyewa' => $id])->row_array();
    ...
    public function hapusProfile(){
        ...
        if(password_verify($password, $cek['password'])){
            ...
            $this->session->set_flashdata('flash','<div class="alert alert-success" style="margin-top:20px; width:325px" role="alert">Akun anda <strong>berhasil dihapus</strong><hr>Terima kasih atas partisipasi anda sebagai mitra penyewa tempat pada SRT</div>');
