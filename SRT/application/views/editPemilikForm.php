<?php $this->load->view('page_header_pem')?>
  <div style="margin-left: 330px;margin-right:335px;margin-top: 120px">
    <ul class="nav nav-pills nav-justified" id="myTab" role="tablist">
      ...
      <li class="nav-item">
        <a class="nav-link" id="penyewa-tab" onclick="profileOption(event)" data-toggle="tab" href="#passProfile" role="tab" aria-selected="false">Ubah Password</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" id="penyewa-tab" onclick="profileOption(event)" data-toggle="tab" href="#hapusProfile" role="tab" aria-selected="false">Hapus Akun</a>
      </li>
    </ul>
  </div>
  ...
  <div class="tab-content">
    <div style="margin-top:25px" class="tab-pane active " id="editProfile" role="tabpanel">
        <form method="post" action="<?= base_url('index.php/pemilikControl/editProfile')?>">
            
            <div class="txt-login">Edit Profile Pemilik Tempat</div>
            <input type="text" class="form-control" name="id" hidden value="<?= $this->session->userdata('idPemilik')?>"/>  
            
            <div style="margin-bottom: 15px">
                <label>Nama Lengkap</label>
                <input type="text" class="form-control" name="nama" value="<?= $this->session->userdata('namaPemilik')?>"/>
            </div>
            ...
            <div style="margin-bottom: 15px">
                ...
                <textarea name="alamat" class="form-control"><?= $this->session->userdata('alamatPemilik')?></textarea>
            </div>
    ...
    <div style="margin-top:25px" class="tab-pane" id="passProfile" role="tabpanel">
        <form method="post" action="<?= base_url('index.php/pemilikControl/editPassProfile')?>">
            <div class="txt-login">Ubah Password</div>
            <input type="Password" class="form-control" name="id" hidden value="<?= $this->session->userdata('idPemilik')?>"/>  
    ...
    <div style="margin-top:25px" class="tab-pane" id="hapusProfile" role="tabpanel">
        <form method="post" action="<?= base_url('index.php/pemilikControl/delProfile')?>">
